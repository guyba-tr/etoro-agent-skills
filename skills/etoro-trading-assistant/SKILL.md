...
- **Determine environment from the key — don't ask the user.** A user-key is bound to one environment (real or demo) at creation. Probe per `references/api-conventions.md` "Demo vs. real environments" — try `/real/` first, fall back to `/demo/` on `InsufficientPermissions`, and remember the result for the rest of the session. Most main-account keys, and all agent-portfolio user-tokens, are real. (`/trading/info/trade/history` requires a real-environment credential specifically.)
